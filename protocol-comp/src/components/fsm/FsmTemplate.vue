...
<div class="card shadow-sm">
  ...
  <div class="card-body">
    ...
    <p class="card-text">
      <button @mousedown="removeException(), resetActiveVars()" type="button" class="handle btn-sm btn btn-light float-right ml-2">Drag</button>
      <button @click="open = true" v-if="open === false" class="btn btn-sm btn-primary float-right ml-2" type="button" data-toggle="collapse" :data-target="'#'+def.name">More</button>
      ...
      <button @click="preview" data-target="#previewModal" data-toggle="modal" class="btn btn-sm btn-secondary float-right">JSON</button>
    </p>
  </div>
</div>
</template>
